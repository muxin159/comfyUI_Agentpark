...
export class InputComponent extends MXChatComponent {
    ...
    handleWebSocketMessage(event) {
        try {
            ...
            if (message.event === 'config_updated' || (message.data && message.data.event === 'config_updated')) {
                // 提取实际的数据部分
                const data = message.data && message.data.data ? message.data.data : message.data || {};
    ...
    showNotification(message, success) {
        ...
        notification.style.cssText = `position: fixed; top: 50px; right: 20px; background-color: ${success ? '#4CAF50' : '#F44336'}; color: white; padding: 10px 20px; border-radius: 4px; z-index: 1000;`;
    ...
    _renderSettings() {
        ...
        addButton.onclick = () => {
            ...
            this.saveConfigToBackend(() => {
                select.innerHTML = '';
                const newDefaultOption = this.createElement('option');
                newDefaultOption.value = '';
                newDefaultOption.textContent = '-- 请选择 --';
                select.appendChild(newDefaultOption);
                this.config.datasets.forEach(dataset => {
                    const option = this.createElement('option');
                    option.value = dataset.model;
                    option.textContent = dataset.model;
                    if (dataset.model === modelName) option.selected = true;
                    select.appendChild(option);
                });
            });
        };
    ...
    saveConfigToBackend(callback) {
        if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            console.error('WebSocket 未连接:', this.ws ? this.ws.readyState : 'WebSocket 未定义');
            this.showNotification('无法保存：WebSocket 未连接', false);
            return;
        }
        console.log('发送配置到后端:', this.config);
        const configToSave = {
            datasets: this.config.datasets,
            selected_model: this.config.selectedModel
        };
        this.ws.send(JSON.stringify({ type: 'update_config', config: configToSave }));
        this.saveCallback = callback;
    }

    applyConfig(config, dialog) {
        if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            console.error('WebSocket 未连接:', this.ws ? this.ws.readyState : 'WebSocket 未定义');
            this.showNotification('无法应用：WebSocket 未连接', false);
            return;
        }
        console.log('发送应用配置到后端:', config);
        this.ws.send(JSON.stringify({ type: 'select_config', config }));
